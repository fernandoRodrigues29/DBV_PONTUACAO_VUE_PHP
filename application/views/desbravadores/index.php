...
const ListagemDesbravadores = {
    ...
    computed: {
        desbravadoresFiltrados() {
            return this.desbravadores.data.filter(d => {
    ...
    mounted(){
        // this.filtros.unidade = this.unidades;
    },
    template: `
        ...
        <!-- Cabeçalho -->
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <select v-model="filtros.unidade" class="form-control">
                                ...
                                <option v-for="u in unidades.data" :key="u.id_unidade" :value="u.id_unidade">
                                    {{ u.nome_unidade }}
                                </option>
                            </select>
                        </div>
            ...
            <div v-else class="row">
                ...
                <div v-for="desb in desbravadoresFiltrados" :key="desb.id_desbravador"
    ...
    methods: {
        ...
        async salvar(dados) {
            try {
                ...
                const url = dados.id_desbravador
                    ? '<?= site_url('desbravadores/atualizar') ?>'
                    : '<?= site_url('desbravadores/inserir') ?>';
        ...
        async confirmarExclusao(desbravador) {
            if (confirm(`Tem certeza que deseja excluir ${desbravador.nome_completo}?`)) {
                try {
                    const resp = await fetch('<?= site_url('desbravadores/deletar') ?>', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                        body: JSON.stringify({ id_desbravador: desbravador.id_desbravador })
                    });
